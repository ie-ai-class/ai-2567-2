...
if __name__ == "__main__":
    ...
    while c != b"\r":  # stop when user preses enter
        if (
            c != b"\t"
        ):  # if previous character was tab, then it write 1st suggestion no wait for inputing
        ...
        if c == b" ":
            ...
            if len(tkns) < 2:  # if 2nd word want to write after 1st
                ...
                last_suggestion = next_word(
                    (tkns[-2], tkns[-1])
                )  # beacaue its a try gram model so we need to last and last to last word
                print(last_suggestion, end=" ", flush=True)
